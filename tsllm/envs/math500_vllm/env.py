...
def extract_answer(completion):
    begin_answer_tag = "<|channel|>final<|message|>"
    end_answer_tag = '<|return|>'
    answer_index = completion.rfind(begin_answer_tag) + len(begin_answer_tag)
    if answer_index < 0:
        return INVALID_ANS

    answer_string = completion[answer_index:]
    if answer_string.endswith(end_answer_tag):
        answer_string = answer_string[:-len(end_answer_tag)]

    substrings = answer_string.split('Answer:')
    if len(substrings) < 2:
        return INVALID_ANS

    return WHITESPACE.sub("", substrings[1])
