...
class BaseMCTSTrainer(ABC):
    def __init__(self, config):
        ...
        if not self.tokenizer.pad_token:
            raise ValueError('PAD token must be set in tokenizer!')
    ...
    def setup_model(self):
        ...
        if self.config.model.model_path == "openai/gpt-oss-20b":
            ...
            model_kwargs.update(dict(attn_implementation="eager", quantization_config=quantization_config,))

        return AutoModelForCausalLMWithValueHead.from_pretrained(self.config.model.model_path, **model_kwargs)
